...
Revision ID: 0fbf615786ad
Revises: 
Create Date: 2025-09-18 00:06:42.368974
...
def upgrade() -> None:
    ...
    op.create_table('milvus_collections',
    sa.Column('id', sa.Integer(), sa.Identity(always=False), nullable=False),
    ...
    sa.Column('search_method', sa.Enum('FLAT', 'HNSW', 'IVF_SQ8', name='search_category'), nullable=False),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['collection_id'], ['milvus_collections.id'], onupdate='CASCADE', ondelete='RESTRICT'),
    sa.ForeignKeyConstraint(['user_id'], ['user_clients.id'], onupdate='CASCADE', ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'category', name='idx_unique_kb_category')
    )
    op.create_table('searching_batch_jobs',
    sa.Column('id', sa.BigInteger(), sa.Identity(always=False), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_table('searching_batch_jobs')
